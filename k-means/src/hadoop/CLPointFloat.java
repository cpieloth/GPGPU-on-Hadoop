...
public class CLPointFloat implements ICLPointOperation<Float> {
	...
	private final KMMapperReducerCL.KMMapper.Context context;
	...
	private List<PointWritable> points;
	...
	public CLPointFloat(CLInstance clInstance, int dim, KMMapperReducerCL.KMMapper.Context context) {
		this(clInstance, dim, 65536, context);
	}

	public CLPointFloat(CLInstance clInstance, int dim, int bufferItems, KMMapperReducerCL.KMMapper.Context context) {
		...
		points = new LinkedList<PointWritable>();
	...
	@Override
	public void put(PointWritable p) {
	...
	private void doNearestPoints() {
		...
		try {
			...
			for (PointWritable p : points) {
				...
				context.write(centroids.get(res.get()), p);
			}
			points.clear();
		} catch (CLException err) {
			Logger.logError(CLAZZ, "OpenCL error:\n" + err.getMessage() + "():"
