...
K_SIZES = ["128", "512", "2048"]    # TODO to set
...
def copyHdfs(data, k):
    ...
    p.wait()
    command = HADOOP_CMD + " fs -put " + DATA_PATH + "/" + "points_" + data + "mb_" + k + "k" + " " + HDFS_INPUT + "/points"
    args = shlex.split(command)
    p = subprocess.Popen(args, stdout=subprocess.PIPE)
    p.wait()
    ...
    p.wait()
    # TODO
    command = HADOOP_CMD + " fs -put " + DATA_PATH + "/" + "center_" + data + "mb_" + k + "k" + " " + HDFS_CENTROIDS + "/center"
    args = shlex.split(command)
    p = subprocess.Popen(args, stdout=subprocess.PIPE)
    p.wait()
...
for data in DATA_SIZES:
    ...
    for k in K_SIZES:
        ...
        for mode in MODES:
            ...
            jobName = "KMeans_" + data + "mb_" + k + "k_" + mode
